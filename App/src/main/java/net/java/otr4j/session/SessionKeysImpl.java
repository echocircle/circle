...
class SessionKeysImpl implements SessionKeys
{
    ...
    public void setLocalPair(KeyPair keyPair, int localPairKeyID)
    {
        ...
        {
            Log.d(ImApp.LOG_TAG, keyDescription + " current local key ID: " + this.getLocalKeyID());
        }
        this.reset();
    }
    ...
    public void setRemoteDHPublicKey(DHPublicKey pubKey, int remoteKeyID)
    {
        ...
        if (Debug.DEBUG_ENABLED)
        {
            Log.d(ImApp.LOG_TAG, keyDescription + " current remote key ID: " + this.getRemoteKeyID());
        }
        this.reset();
    }
    ...
    public void incrementSendingCtr()
    {
        if (Debug.DEBUG_ENABLED)
        {
            Log.d(ImApp.LOG_TAG, "Incrementing counter for (localkeyID, remoteKeyID) = (" + getLocalKeyID()
                    + "," + getRemoteKeyID() + ")");
        }
    ...
    private void reset()
    {
        if (Debug.DEBUG_ENABLED)
        {
            Log.d(ImApp.LOG_TAG, "Resetting " + keyDescription + " session keys.");
        }
    ...
    public byte[] getSendingAESKey() throws OtrException
    {
        ...
        if (Debug.DEBUG_ENABLED)
        {
            Log.d(ImApp.LOG_TAG, "Calculated sending AES key.");
        }
    ...
    public byte[] getReceivingAESKey() throws OtrException
    {
        ...
        if (Debug.DEBUG_ENABLED)
        {
            Log.d(ImApp.LOG_TAG, "Calculated receiving AES key.");
        }
    ...
    public byte[] getSendingMACKey() throws OtrException
    {
        ...
        if (Debug.DEBUG_ENABLED)
        {
            Log.d(ImApp.LOG_TAG, "Calculated sending MAC key.");
        }
        return sendingMACKey;
    }
    ...
    private BigInteger getS() throws OtrException
    {
        if (s == null)
        {
            ...
            if (Debug.DEBUG_ENABLED)
            {
                Log.d(ImApp.LOG_TAG, "Calculating shared secret S.");
            }
        }
        return s;
    }
